...
<div id="desc" style="height: auto; font-size: 50px; text-transform: uppercase;"> @if(isset($prod_id)) {{ $prod->descripcion }} <br> <p style='font-weight: bold;'> Kg:  {{ $prod->pesoRef }}  Diam: {{ $prod->diametroRef }}  Mts: {{ $prod->metrosRef }} </p>  @endif </div>
