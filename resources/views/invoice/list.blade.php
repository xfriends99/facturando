	...
	<div class="row">
		<div class="col-md-8 col-md-offset-2">
			<div class="panel panel-default">
				<div class="panel-heading">Administrar Facturas
					</div>
					...
					<table class="table table-hover">
						...
						<tbody>
							...
							<tr>
								...
								<th>{{ $invoice->nro_cbte  }}</th>
								<td>{{ $invoice->company_name }}</td>
								<td>{{ '$ '.number_format($invoice->imp_total,2) }}</td>
								<td>{{ \myFunctions::FacturaStatus($invoice->status) }}</td>
								<td>@if($invoice->status=='G')
									<a href= "/editarFactura/{{$invoice->id}}" class="btn btn-info" >Editar</a>&nbsp;&nbsp;<a href= "/emitirFactura/{{$invoice->id}}" class="btn btn-success" >Emitir Cbte.</a>&nbsp;&nbsp;<a href= "/eliminarFactura/{{$invoice->id}}" onClick="return confirm('¿Esta seguro?');" class="btn btn-danger" >Eliminar</a>
									@endif
									@if($invoice->status=='A')
									<a href= "/verFactura/{{$invoice->id}}" class="btn btn-info" >Ver Cbte.</a>
									<a href= "/descargarFactura/{{$invoice->id}}" class="btn btn-warning" >Descargar Cbte.</a>
